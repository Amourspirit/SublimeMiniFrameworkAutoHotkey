...
class ahkrun(sublime_plugin.WindowCommand):
	def run(self):
		...
		AutoHotKeyExePathList = sublime.load_settings("AutoHotkey-MF.sublime-settings").get("AutoHotKeyExePath")
		AutoHotKeyExePath = ""
		...
		if not os.path.isfile(AutoHotKeyExePath):
			AutoHotKeyExePath = sublime.load_settings("AutoHotkey-MF.sublime-settings").get("AutoHotKeyExePath")["default"]
...
class ahkcompile(sublime_plugin.WindowCommand):
	def run(self):
		...
		Ahk2ExePathList = sublime.load_settings("AutoHotkey-MF.sublime-settings").get("Ahk2ExePath")
		Ahk2ExePath = ""
		...
		if not os.path.isfile(Ahk2ExePath):
			Ahk2ExePath = sublime.load_settings("AutoHotkey-MF.sublime-settings").get("Ahk2ExePath")["default"]
...
class ahkrunpipedCommand(ahkrunpiped):
	...
	def run(self, edit, version='default'):
		...
		if not os.path.isfile(AutoHotKeyExePath):
			self.ahkpath = sublime.load_settings("AutoHotkey-MF.sublime-settings").get("AutoHotKeyExePath")["default"]
